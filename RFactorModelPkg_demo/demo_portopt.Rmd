...
#multiple factors
factorIDs <- c("F000003","F000006","F000008","F000021")
FactorLists1 <- buildFactorLists_lcfs(factorIDs,factorRefine=refinePar_default("old_fashion"))
...
fRtn <- data.frame(fname=c("F_NP_chg_w13","ln_mkt_cap_","NP_YOY","PB_mrq_","pct_chg_per_60_","pio_f_score"),
                   frtn=c(0.003,0.001,0.005,0.005,0.002,0.001),stringsAsFactors = FALSE)
...
constr2 <- addConstr_fctExp_style(constr2,conslist,rep(-0.01,6),c(100,100,0.5,1,0.1,100),relative = 0)
...
#control stock's weight which in specified universe
constr <- addConstr_group(constr,EI000905=c(0.8,0.95))

#control industry's exposure 
constr <- addConstr_fctExp_sector(constr,each = c(-0.1,0.1))

#control style factor's exposure 
conslist <- FactorLists
constr <- addConstr_fctExp_style(constr,conslist,rep(-0.01,6),c(100,100,0.5,1,0.1,100),relative = 0)
...
## object setting
obj <- object_default()
obj <- addObj_risk(obj,risk_aversion = 10)
...
#max return minus risk
port_opt <- getPort_opt(TSF,fRtn = fRtn,fCov=fCov,bmk="EI000905",constr = constr,obj = obj)

#max return minus risk with trackingerror target
port_opt2 <- getPort_opt(TSF,fRtn = fRtn,fCov=fCov,bmk="EI000905",constr = constr,obj = obj,delta=delta)
